...
export default {
  ...
  methods: {
    ...
    getCaptcha (e) {
      ...
      validateFields(['mobile'], { force: true }, (err, values) => {
        if (!err) {
          ...
          const hide = this.$message.loading('Ładuję..', 0)
          getSmsCaptcha({ mobile: values.mobile }).then(res => {
            setTimeout(hide, 2500)
            this.$notification['success']({
              message: 'Sukces',
              description: 'Dane z ReCaptcha：' + res.result.captcha,
              duration: 8
            })
          }).catch(err => {
            setTimeout(hide, 1)
    ...
    loginSuccess (res) {
      console.log(res)
      console.log(storage.get(APP_LANGUAGE, 'pl-PL'))
      this.$router.push({ path: '/' })
      setTimeout(() => {
        this.$notification.success({
          message: 'Sukces',
          description: 'Jest ' + new Date().toLocaleTimeString(storage.get(APP_LANGUAGE, 'pl-PL'), { hour12: false, hour: 'numeric', minute: 'numeric' }) + ` ${timeFix()}`
        })
      }, 1000)
    ...
    requestFailed (err) {
      ...
      this.$notification['error']({
        message: 'Błędne dane',
        description: ((err.response || {}).data || {}).message || 'Spróbuj ponownie',
        duration: 4
      })
    }
  }
